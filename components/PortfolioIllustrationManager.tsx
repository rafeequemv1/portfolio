...
import { PortfolioIllustration } from '../types';
...
const MAX_ILLUSTRATION_PIXEL = 3200;
...
async function fileToWebpBlob(file: File): Promise<Blob> {
  ...
  let w = bmp.width;
  let h = bmp.height;
  if (w > MAX_ILLUSTRATION_PIXEL || h > MAX_ILLUSTRATION_PIXEL) {
    const s = Math.min(MAX_ILLUSTRATION_PIXEL / w, MAX_ILLUSTRATION_PIXEL / h);
    w = Math.round(w * s);
    h = Math.round(h * s);
  }
...
const PortfolioIllustrationManager: React.FC = () => {
  const [rows, setRows] = useState<PortfolioIllustration[]>([]);
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);
  const [uploading, setUploading] = useState(false);
  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing] = useState<PortfolioIllustration | null>(null);
  ...
  const openEdit = (row: PortfolioIllustration) => {
    setEditing(row);
    setForm({
      title: row.title || '',
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of Array.from(files)) {
      try {
        ...
        const path = `illustrations/${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editing) {
      const { error } = await supabase.from('portfolio_illustrations').update(payload).eq('id', editing.id);
      if (error) alert(error.message);
    } else {
      const { error } = await supabase.from('portfolio_illustrations').insert(payload);
      if (error) alert(error.message);
    }
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Delete this illustration?')) return;
    const { error } = await supabase.from('portfolio_illustrations').delete().eq('id', id);
    if (error) alert(error.message);
    else fetchRows();
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-serif text-[#37352f]">Illustrations</h2>
          <p className="mt-1 text-sm text-[#37352f]/60">Standalone artwork shown in the Illustrations portfolio tab.</p>
        </div>
        <button
          type="button"
          onClick={openNew}
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-[#37352f]/15 py-12 text-center text-sm text-[#37352f]/50">No illustrations yet.</div>
      ) : (
          ...
          <div className="fixed inset-0 z-[210] flex items-center justify-center bg-black/45 p-4 backdrop-blur-sm">
            <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-[#37352f]/10 bg-white shadow-2xl">
              <div className="sticky top-0 flex items-center justify-between border-b border-gray-100 bg-white px-6 py-4">
                <h3 className="font-serif text-xl text-[#37352f]">{editing ? 'Edit illustration' : 'Add illustration'}</h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-4 p-6">
                <div>
                  <label className="mb-2 block text-xs font-bold uppercase tracking-wider text-[#37352f]/55">Images</label>
                ...
                <div>
                  ...
                  <textarea rows={3} value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} className="w-full resize-y rounded-lg border border-gray-200 px-4 py-2.5 text-sm" placeholder="Context, medium, project notes…" />
                </div>
                <div>
...
export default PortfolioIllustrationManager;

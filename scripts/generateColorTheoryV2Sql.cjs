/**
 * Emits supabase/color_theory_v2_full_replace.sql
 * Uses jsonb_build_object + dollar quotes only (no ASCII double-quote in file) for easy MCP / tooling.
 */
const fs = require("fs");
const path = require("path");

const outPath = path.join(__dirname, "..", "supabase", "color_theory_v2_full_replace.sql");

let tagSeq = 1;
function nextTag() {
  return `z${String(tagSeq++).padStart(5, "0")}`;
}

function dollar(tag, body) {
  if (body.includes(`$${tag}$`)) throw new Error(`Body contains delimiter ${tag}`);
  return `$${tag}$${body}$${tag}$`;
}

function textBlock(id, text) {
  const t = nextTag();
  return `jsonb_build_object('id','${id}','type','text','text',${dollar(t, text)})`;
}

function htmlBlock(id, html) {
  const t = nextTag();
  return `jsonb_build_object('id','${id}','type','html','html',${dollar(t, html)},'css','')`;
}

function labBlock(id, preset) {
  return `jsonb_build_object('id','${id}','type','color_lab','preset','${preset}')`;
}

const htmlContrast = `<div style='font-family:system-ui,Segoe UI,sans-serif;font-size:14px;line-height:1.5;color:#1e293b'>
...
const htmlUnequal = `<div style='font-family:system-ui,sans-serif;font-size:13px;line-height:1.45;color:#0f172a'>
...
const htmlCategorical = `<div style='font-family:system-ui,sans-serif;font-size:12px;line-height:1.45;color:#334155'>
...
Third, vary how much of each color you use. A strong accent can do more work in a thin stripe than in a half-page flood. When you are ready, go to the next chapter for the classic simultaneous-contrast demo and why your neutrals need a real background.`,
      ),
    ],
  },
  {
    id: "c0c0c000-0002-4000-8000-00000000d001",
    pos: 1,
    title: "Contrast, context, and illusions",
    blocks: () => [
      textBlock(
        "ct-0101",
        `Readers judge a panel long before they read every label. Color is already grouping regions, pushing one trace forward, and telling a subconscious story about what is background versus signal. When colors fight, people often feel uneasy without being able to name the reason.
...
Use the hue ring below the same way in every chapter: pick a hue, then imagine it as a thin accent line or a large field. That imagination step matters because large saturated areas behave very differently from small glyphs.`,
      ),
      labBlock("ct-0102", "hue_wheel"),
      htmlBlock("ct-0103", htmlContrast),
      textBlock(
        "ct-0104",
        `The gray squares in the embed are identical on the page. Your visual system still compares them to their neighbors. That is why pathway neutrals and pastel fills shift when the surround changes.

Try a five-minute drill on your own file. Duplicate one panel, nudge only the background warmer in one copy and cooler in another, and leave the data colors untouched. If a legend label suddenly feels too loud or too shy, you have learned something useful without touching your statistics at all.`,
      ),
    ],
  },
  {
    id: "c0c0c000-0002-4000-8000-00000000d002",
    pos: 2,
    title: "Hue, saturation, and lightness (HSL)",
    blocks: () => [
      textBlock(
        "ct-0201",
...
Every lesson opens with the same ring so you never have to hunt for a wheel. Use it here to feel how one hue can look gentle or loud depending on saturation, and how small lightness moves can rescue a label that felt muddy.`,
      ),
      labBlock("ct-0202", "hue_wheel"),
      textBlock(
        "ct-0203",
        `When you move from screen to print, remember that big fields of full saturation can look almost neon on coated stock while the same choice feels softer on a monitor. A practical habit is to keep your loudest saturation for arrows, outlines, and key markers, and to let broad backgrounds breathe a little.

If you work with collaborators, try naming edits in HSL words. Instead of make it bluer, say raise lightness so it separates from the navy field, or pull saturation down so the heatmap stops shouting. Shared vocabulary shortens revision loops because everyone is turning the same knobs.`,
      ),
    ],
  },
  {
    id: "c0c0c000-0002-4000-8000-00000000d003",
    pos: 3,
    title: "Harmonies for multi-panel figures",
    blocks: () => [
      textBlock(
        "ct-0301",
        `A single frame often carries a schematic, an inset micrograph, and a compact chart. If each region picks a random favorite hue, the reader meets noise before insight. Harmony, in the everyday sense used here, means choosing hues that relate on purpose so the canvas reads as one designed object.
...
Start with the hue ring to pick a base mood, then open the harmony strip to see how relatives rotate with that base. If you can name which swatch maps to control, treatment, or rescue, you are far less likely to choose a hue only because it looked pretty alone.`,
      ),
      labBlock("ct-0302", "hue_wheel"),
      labBlock("ct-0303", "harmonies"),
      textBlock(
        "ct-0304",
        `When you translate harmony into layout, keep one family visually dominant across the largest areas and let secondary hues appear in smaller, sharper doses. Harmony is not only which hues you pick; it is how much of each hue the eye has to process at once.

HTML embeds are still a good place to mock legend spacing and margins. Next you will pair complements for focal contrast, again with the wheel in reach.`,
      ),
    ],
  },
  {
    id: "c0c0c000-0002-4000-8000-00000000d004",
    pos: 4,
    title: "Complements and focal accents",
    blocks: () => [
      textBlock(
        "ct-0401",
        `Complementary hues sit roughly opposite on the wheel. That relationship is powerful because our attention loves hue contrast. Used with care, a complement can highlight one vesicle, one knockout band, or one pathway step without rebuilding your entire palette. Used as two equal neon slabs, the same pair can fatigue people quickly.
...
The wheel below is your anchor; the dedicated complement widget under it keeps saturation and lightness aligned in a simple way so you can rehearse pairs before you paste hex codes into your real layout.`,
      ),
      labBlock("ct-0402", "hue_wheel"),
      labBlock("ct-0403", "complementary"),
      htmlBlock("ct-0404", htmlUnequal),
      textBlock(
        "ct-0405",
        `If a complement feels harsh, try shrinking the loud color first before you change the hue. Unequal area is often the first fix, not a new picker session.

When you like a pair, park the swatches on your actual background and squint. If the relationship still reads, you are ready for the closing chapter on accessibility, temperature cues, and print.`,
      ),
    ],
  },
  {
    id: "c0c0c000-0002-4000-8000-00000000d005",
    pos: 5,
    title: "Accessibility, temperature, and shipping",
    blocks: () => [
      textBlock(
        "ct-0501",
        `Color is part of communication, not only decoration. One common failure is encoding categories with red and green alone, because many readers will not separate those hues the way you expect. The fix is rarely to ban color; it is to add lightness separation, patterns, labels, or redundant cues so the story survives grayscale and common forms of color-vision difference.
...
Use the hue ring to remind yourself that two pretty hues can share the same lightness even when they look worlds apart in RGB. Then look at the schematic strips and the warm-cool strip to keep temperature intentional rather than accidental.`,
      ),
      labBlock("ct-0502", "hue_wheel"),
      htmlBlock("ct-0503", htmlCategorical),
      labBlock("ct-0504", "temperature"),
      textBlock(
        "ct-0505",
        `Warm and cool associations depend on culture and context, but in biomedical graphics you will still see warm cues tied to energy or warning and cool cues tied to structure or calm. Name your defaults so they match the biology story instead of drifting by habit.
...
function emitSql() {
  tagSeq = 1;
  const lines = [];
  lines.push(`-- Color theory course v2: intro chapter + hue_wheel on every chapter + tetradic row in harmony lab (app)`);
  lines.push(`-- Safe to re-run: replaces chapters for slug color-theory-scientific-images only.`);
  lines.push(``);
  lines.push(`update public.courses c`);
  lines.push(`set`);
  lines.push(
  ...
  lines.push(`where c.slug = 'color-theory-scientific-images';`);
  lines.push(``);
  lines.push(
  ...
  lines.push(``);
  lines.push(`insert into public.course_chapters (id, course_id, title, position, content_blocks, updated_at)`);
  lines.push(`values`);

  const valueRows = chapters.map((ch, idx) => {
    const parts = ch.blocks();
    const inner = parts.join(",\n    ");
    const blocks = `jsonb_build_array(\n    ${inner}\n  )`;
    const row = `  (
    '${ch.id}'::uuid,
    ...
    return row + (idx < chapters.length - 1 ? "," : ";");
  });

  lines.push(...valueRows);
  const sql = lines.join("\n");
  if (/"/.test(sql)) throw new Error("SQL must not contain double-quote characters");
  return sql;
}

fs.writeFileSync(outPath, emitSql(), "utf8");
